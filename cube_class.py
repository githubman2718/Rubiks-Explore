...
class Cube:
    ...
    def badness(self):
        #A huristic that attempts to describe how close a cube is to being solved.
        return np.sum(self.cube[self.nonzero] != self.init[self.nonzero])

    def general_q_solve(self):
        #attempts to solve a general cube by implimenting a tree search which minimizes the "badness" heuristic. Doesn't yet work.
        save = self.cube.copy()
        solution = []
        while not self.solved():
            currunt_badness = self.badness()
            print(currunt_badness, self.qdepth())
            look_ahead = []
            stop_condition = False
            while stop_condition == False:
                for rotation in self.qrotations:
                    axis, index, k = rotation
                    self.rotate(axis, index, k)
                    if self.badness() < currunt_badness:
                        look_ahead.append(rotation)
                        solution+=look_ahead
                        stop_condition = True
                        look_ahead = []
                        break
                    ...
                    #print(f'solution = {solution}', f'look ahead = {look_ahead}')
                    if look_ahead == []:
                        look_ahead.append(self.qrotations[0])
                        axis, index, k = look_ahead[-1]
                        self.rotate(axis, index, k)
                    elif look_ahead[-1] != self.qrotations[-1]:
                        axis, index, k = look_ahead[-1]
                        self.rotate(axis, index, -k)
                        index_of_new_rotation = self.qrotations.index(look_ahead[-1]) + 1
                        look_ahead[-1] = self.qrotations[index_of_new_rotation]
                        axis, index, k = look_ahead[-1]
                        self.rotate(axis, index, k)
                    else:
                        look_ahead.reverse()
                        for count, elem in enumerate(look_ahead):
                            if elem != self.qrotations[-1]:
                                for rotation in look_ahead[:count+1]:
                                    axis, index, k = rotation
                                    self.rotate(axis, index, -k)
                                index_of_new_rotation = self.qrotations.index(look_ahead[count]) + 1
                                look_ahead[count] = self.qrotations[index_of_new_rotation]
                                look_ahead[:count] = [self.qrotations[0] for _ in look_ahead[:count]]
                                look_ahead.reverse()
                                for rotation in look_ahead[-count:]:
                                    axis, index, k = rotation
                                    self.rotate(axis, index, k)
                                break
                        else:
                            for rotation in look_ahead:
                                axis, index, k = rotation
                                self.rotate(axis, index, -k)
                            look_ahead = [self.qrotations[0] for _ in look_ahead]
                            look_ahead.append(self.qrotations[0])
                            for rotation in look_ahead:
                                axis, index, k = rotation
                                self.rotate(axis, index, k)
